...
def test_is_King_in_check():
    ...
    test_board = parse_fen(
        "rnb1kbnr/pppp1ppp/8/4p3/3P3q/4PP2/PPP3PP/RNBQKBNR w KQkq - 0 1"
    )
    assert test_board.is_in_check("white")
    assert not test_board.is_in_check("black")

    test_board = parse_fen(
        "r1b1kbnr/ppp2ppp/3p1q2/2P1p3/8/3nPPP1/PPP4P/RNBQKBNR w KQkq - 0 1"
    )
    assert test_board.is_in_check("white")
    assert not test_board.is_in_check("black")

    test_board = parse_fen(
        "rR2k2r/p3bp1p/3pnqp1/2P1p3/7N/3QPPPB/P1P4P/4K2R w Kkq - 0 1"
    )
    ...
    test_board = parse_fen(
        "rnbqk1nr/pppp1ppp/4p3/8/1b1P4/8/PPP1PPPP/RNBQKBNR w KQkq - 0 1"
    )
    assert test_board.is_in_check("white")
    assert not test_board.is_in_check("black")
